...
class App:
    ...
    def drop_piece(self, row, col, piece):
        if (self.board[ROW_COUNT-1-row][col])["bg"] == "red" or (self.board[ROW_COUNT-1-row][col])["bg"] == "yellow":
           row = row + 1
        if(piece == self.redCoin):
            (self.board[ROW_COUNT-1-row][col])["bg"] = "red"
        else:
            (self.board[ROW_COUNT-1-row][col])["bg"] = "yellow"
